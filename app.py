...
def render_sidebar():
    ...
    params = {
        ...
        "stoch_d":    stoch_d,
        "willr_len":  willr_len,
        "subset":     selected_subset if selected_subset else all_stocks,
    }
    return params
...
def build_screening_table(tickers_tuple, period, interval,
    ...
    """Build summary table for all tickers. Cached per parameter set."""
    rows = []
    ...
    for ticker in tickers:
        ...
        if df.empty or len(df) < 30:
            rows.append({
                ...
                "Signal": "N/A", "Score": 0,
                **{k: 0 for k in ["RSI","MACD","CCI","Momentum","Stochastic","Williams%R"]},
            })
            continue
...
def render_page1(params):
    ...
    st.markdown(
        f"Showing **{len(params['subset'])} stocks** • Timeframe: **{params['freq']}** • "
        f"Signals based on 6 indicators (max score ±12)"
    )
    ...
    with st.spinner(""):
        df_all = build_screening_table(
            tuple(tickers), params["period"], params["interval"],
    ...
    header_html = "<table style='width:100%; border-collapse:collapse; font-size:0.83rem;'>"
    header_html += "<thead><tr style='border-bottom:2px solid #2d3748; color:#718096;'>"
    for h in headers:
        header_html += f"<th style='padding:0.5rem 0.6rem; text-align:left;'>{h}</th>"
    ...
    for _, row in display_df.iterrows():
        ...
        header_html += "<tr style='border-bottom:1px solid #1a202c;'>"
        header_html += f"<td style='padding:0.45rem 0.6rem; font-weight:600; color:#63b3ed;'>{row['Ticker']}</td>"
        header_html += f"<td style='padding:0.45rem 0.6rem; color:#a0aec0; font-size:0.78rem;'>{row['Name']}</td>"
        header_html += f"<td style='padding:0.45rem 0.6rem;'>{price_str}</td>"
        header_html += f"<td style='padding:0.45rem 0.6rem; color:{chg_color}; font-weight:600;'>{chg_str}</td>"
        header_html += f"<td style='padding:0.45rem 0.6rem;'>{signal_badge_html(row['Signal'])}</td>"
        ...
        for ic in ind_cols:
            header_html += f"<td style='padding:0.45rem 0.6rem;'>{score_to_bar(int(row.get(ic, 0)))}</td>"
        header_html += "</tr>"
    ...
    st.download_button("⬇ Export to CSV", data=csv, file_name="bist_ta_signals.csv", mime="text/csv")
